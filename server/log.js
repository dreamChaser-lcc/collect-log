    ...
    _.each(storages, function (appStorage, app) {
        _.each(appStorage, function (typeStorage, type) {
            _.each(typeStorage, function (timeStorage, time) {
                ...
                sendLogs(logs, function (err, body) {
                    if (err) {
                        console.error('send logs error. time: ', time, ' type:', type, ' log count: ', count);
                        console.error(err);
                        return;
                    }
                    ...
                    if (body.state && body.state.code === 0) {
                        ...
                        console.error('send logs failed. time: ', time, ' type:', type, ' log count: ', count, ' response:', body);
                    }
                });
            });
        });
...
function sendLogs (logs, cb) {
    ...
    proxy.apiProxy(conf.logApi, params, cb, conf.logApiSecret);
}
...
function init(_dir) {
    ...
    // 每隔一段时间（10秒）将数据写入硬盘
    setTimeout(function () {
        setInterval(dumpToServer, 15000);
